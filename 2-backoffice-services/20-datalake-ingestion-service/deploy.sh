...
sam deploy --debug --stack-name $SAM_STAGE-wrbs-boff-dlin --capabilities CAPABILITY_NAMED_IAM --s3-bucket $SAM_BUCKET_NAME --profile $SAM_AWS_PROFILE --region $SAM_AWS_REGION --tags Stage=$SAM_STAGE Workload=wrbs Context=boff Service=dlin WorkloadLongName=wild-rydes-backend-services ContextLongName=backoffice-services ServiceLongName=datalake-ingestion-service
