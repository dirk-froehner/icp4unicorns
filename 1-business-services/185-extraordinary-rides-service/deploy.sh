...
sam deploy --debug --stack-name $SAM_STAGE-wrbs-busi-exri --capabilities CAPABILITY_IAM --s3-bucket $SAM_BUCKET_NAME --profile $SAM_AWS_PROFILE --region $SAM_AWS_REGION --tags Stage=$SAM_STAGE Workload=wrbs Context=busi Service=exri WorkloadLongName=wild-rydes-backend-services ContextLongName=business-services ServiceLongName=extraordinary-rides-service
